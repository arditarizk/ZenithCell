// ==========================================
// MASTER API ZENITH CELL (V38 - FIX ARRAY INDEXING BUG)
// ==========================================
...
function doPost(e) {
  ...
  try {
    ...
    if (payload.tipe === "KAS_MASUK_CICILAN") {
      ...
      var isFound = false;
      var rowIndex = -1;
      var targetRowData = null; // MENGGUNAKAN KOORDINAT MUTLAK

      for (var i = 1; i < dL.length; i++) {
        if (cleanId(dL[i][0]) === cleanId(payload.idKontrak)) {
          isFound = true; 
          rowIndex = i + 1; 
          targetRowData = dL[i]; // Ambil data baris tersebut langsung
          break;
        }
      }
      ...
      // Menggunakan array targetRowData, BUKAN rowIndex-2 lagi!
    ...
    if (payload.tipe === "PELUNASAN_AWAL") {
      ...
      for (var i = 1; i < dL.length; i++) {
        if (cleanId(dL[i][0]) === cleanId(payload.idKontrak)) {
          isFound = true; rowIndex = i + 1; targetRowData = dL[i]; break;
        }
      }
      ...
      try {
          ...
          if (emailTujuan && emailTujuan.indexOf('@') > -1) {
              MailApp.sendEmail(emailTujuan, "✅ LUNAS FULL: " + payload.nama, 
    ...
    return createJsonResponse({status: "error", msg: "Server Error: " + err.toString()}); 
  } finally {
    lock.releaseLock();
  }
}
  ...
  try {
    ...
    if (e.parameter.wa) {
      ...
      if (sR && sR.getLastRow() >= 2) {
        ...
        for (var k = 1; k < dR.length; k++) {
          if (String(dR[k][2]).replace(/[^0-9]/g, '') === sw) {
            ...
            historyLoans.push({ idKontrak: cleanId(dR[k][0]), barang: dR[k][3], tanggalLunas: dR[k][5] || "Telah Lunas" });
          }
        }
      }
      ...
      if(activeLoans.length > 0 || historyLoans.length > 0 || pendingLoans.length > 0) {
          ...
          if (historyLoans.length > 0 && totalTelat === 0) { skorData = { score: "A+", badge: "VVIP Member" }; } 
