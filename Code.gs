// ==========================================
// MASTER API ZENITH CELL (V17 - ANTI BUG STRING/NUMBER)
// ==========================================
...
function doPost(e) {
  try {
    ...
    if (payload.tipe === "PENGAJUAN_BARU") {
      ...
      // Tambahkan tanda petik (') agar angka 13 digit murni dibaca sebagai Teks oleh Google Sheets
      s.appendRow([
    ...
    if (payload.tipe === "ACC_PENGAJUAN") {
      ...
      if (sP) {
        ...
        for (var i = 1; i < dP.length; i++) {
          // BUG FIX: Gunakan String() untuk memaksa pencocokan Teks vs Teks
      ...
      if (sL) {
        ...
        for (var i = 1; i < dL.length; i++) {
          if (String(dL[i][0]) === String(payload.idKontrak)) {
            ...
            sL.deleteRow(i + 1); // Dihapus dari Tagihan Aktif karena sudah lunas
            break;
          }
        }
      }
  ...
  try {
    ...
    if (e.parameter.wa) {
      ...
      if (s && s.getLastRow() >= 2) {
        var d = s.getDataRange().getValues();
        for (var i = 1; i < d.length; i++) {
          // Konversi ke string agar nomor yang diawali petik tetap terbaca
